...
class ExcusedController extends Controller
{
    public function index(Request $request) {

        if($request->ajax()){

            $excused_start_date = $request->get('excused_start_date') ? $request->get('excused_start_date') : '1900-01-01';
            $excused_end_date = $request->get('excused_end_date') ?  $request->get('excused_end_date') : '2099-12-31';
            $excused_reason_id = $request->get('excused_reason_id') ? $request->get('excused_reason_id') : '';

            $users = User::with('employees')
            ->whereHas('employees');
            })
                ->when($date_sent_from, function ($query) use($date_sent_from, $date_sent_to) {
